...
def build_payload(data: dict, application: str, owner: str, repo: str,
                  host: str = "github.com", routes: list | None = None) -> dict:
    ...
    # AEX (CP6): rotas mapeadas por parse_routes.py. Só entram se houver — sem rotas o
    # payload segue idêntico ao de antes (não quebra repositório sem código Python).
    if routes:
...
def main() -> int:
    ...
    ap.add_argument("--routes", default="routes.json",
                    help="mapa de rotas (parse_routes.py) p/ o AEX; ausente = sem rotas")
    ...
    # Rotas para o AEX (opcional): arquivo ausente/ilegível => segue sem rotas (não quebra).
    rotas = None
    try:
        rotas = json.loads(open(args.routes, encoding="utf-8").read()).get("routes") or None
    except (OSError, json.JSONDecodeError, AttributeError):
        rotas = None

    payload = build_payload(data, args.application or args.repo, args.owner,
                            args.repo, args.host, routes=rotas)
    n = len(payload["findings"])
    nr = len(payload.get("routes") or [])
    by_cat = {}
    ...
    try:
        ...
        print(f"[signal-push] não consegui contatar o Signal ({type(e).__name__}). "
              f"Achados: {n} ({resumo}).")
        return 0
    print(f"[signal-push] enviados {n} achado(s) ao Signal [{resumo}]"
          f"{f' + {nr} rota(s)' if nr else ''} · HTTP {status}.")
    return 0
